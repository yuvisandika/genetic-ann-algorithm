class Player {
  ...
  int lifespan = 0;//berapa lama pelayer hidup (untuk fitness)
  int bestScore = 0;//menyimpan skor yang dicapai buat untuk replay
  boolean dead;
  int score;
  int gen = 0;
  ...
  float[] vision = new float[genomeInputs];//array input dimasukkan ke dalam NN
...
//????
  ...
  //---------------------------------------------------------------------------------------------------------------------------------------------------------
  //jika parameter benar dan ada di udara akan meningkatkan gravitasi (nunduk goblok)
  ...
  void look() {
    if (!replay) {
      ...
      boolean berd = false; 
      for (int i = 0; i< obstacles.size(); i++) {
        if (obstacles.get(i).posX + obstacles.get(i).w/2 - (playerXpos - dinoRun1.width/2) < min &&  obstacles.get(i).posX + obstacles.get(i).w/2 - (playerXpos - dinoRun1.width/2) > 0) {//jika jarak antara kiri pemain dan kanan rintangan adalah yang paling sedikit
      ...
      for (int i = 0; i< birds.size(); i++) {
        if (birds.get(i).posX + birds.get(i).w/2 - (playerXpos - dinoRun1.width/2) < min &&  birds.get(i).posX + birds.get(i).w/2 - (playerXpos - dinoRun1.width/2) > 0) {//jika jarak antara kiri pemain dan kanan rintangan adalah yang paling sedikit
      ...
      if (minIndex == -1) {//if there are no obstacles run dino run
        vision[0] = 0; 
        vision[1] = 0;
        vision[2] = 0;
        vision[3] = 0;
        ...
        //vision 6 - renggan ke obstacle  berikutnya
      ...
      if (minIndex == -1) {//if there are no replayObstacles back to run dino
        vision[0] = 0; 
        vision[1] = 0;
        vision[2] = 0;
        vision[3] = 0;
        ...
        //vision 6 ini kalo ngga salah renggang ke obstacle  berukutnya berikutnya lagi
        int bestIndex = minIndex;
        float closestDist = min;
        min = 10000;
        minIndex = -1;
  ...
  //---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // karena ada beberapa keacakan dalam game kadang-kadang ketika kita ingin memutar ulang permainan kita perlu menghapus keacakan itu
  // fungsinya ini
  ...
  //--------------------------------------------------------------------------------------------------------------------------------------------------------
  //jika memutar ulang maka dino memiliki kendala lokal
