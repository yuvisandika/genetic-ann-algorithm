class Species {
  ...
  int staleness = 0;//berapa generasi spesies yang mati tanpa perbaikan
  Genome rep;

  //--------------------------------------------
  //koefisien untuk menguji kompatibilitas
  float excessCoeff = 1;
  ...
  //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
  //constructor  yang mengambil player yang termasuk dalam spesies
  Species(Player p) {
    players.add(p); 
    //karena ini adalah satu-satunya spesies yang secara default adalah yang terbaik
  ...
  boolean sameSpecies(Genome g) {
    float compatibility;
    float excessAndDisjoint = getExcessDisjoint(g, rep);//get jumlah gen berlebih dan terpisahkan antara pemain ini dan rep spesies saat ini
    float averageWeightDiff = averageWeightDiff(g, rep);//get perbedaan berat rata-rata antara gen yang cocok
  ...
  //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
  // mengembalikan jumlah gen berlebih dan terputus-putus antara 2 genom input
  //yaitu. mengembalikan jumlah gen yang tidak cocok
  float getExcessDisjoint(Genome brain1, Genome brain2) {
    ...
    return (brain1.genes.size() + brain2.genes.size() - 2*(matching));//return tidak ada gen berlebih dan terpisah
  }
  //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  //returns perbedaan berat rata-rata antara gen yang cocok dalam genom input
  ...
  Player giveMeBaby(ArrayList<connectionHistory> innovationHistory) {
    ...
    if (random(1) < 0.25) {//25% of the time there is no crossover and the child is simply a clone of a random(ish) player
      ...
      //fungsi crossover mengharapkan orang tua kebugaran tertinggi untuk menjadi objek dan terendah sebagai argumen
  ...
  //------------------------------------------------------------------------------------------------------------------------------------------
  //kills off bottom half of the species mati lu goblok
  void cull() {
    if (players.size() > 2) {
  ...
  //------------------------------------------------------------------------------------------------------------------------------------------
  //untuk melindungi pemain unik, kebugaran setiap pemain dibagi dengan jumlah pemain dalam spesies yang dimiliki pemain itu 
  void fitnessSharing() {
    for (int i = 0; i< players.size(); i++) {
